...
class depthConv(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, p=1, g=1, d=1, act=True, bias=True):
        super(depthConv, self).__init__()
        print(f"depthConv: in_channels={c1}, out_channels={c2}")
        # self.depthwise = nn.Conv2d(c1, c1, kernel_size=k, stride=s, padding=p, groups=c1, bias=False)
        # self.pointwise = nn.Conv2d(c1, c2, kernel_size=1, padding=0, bias=False)
        # self.batchnorm = nn.BatchNorm2d(c2, affine=True)
        # self.re = nn.SiLU()
        self.depthwise = nn.Conv2d(c1, c1, kernel_size=k,
                                   stride=s, padding=p, groups=c1, bias=False)
...
class DP_Conv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.depth_conv = nn.Conv2d(c1, c1, kernel_size=3, stride=1, padding=1, groups=c1)
        self.point_conv = nn.Conv2d(c1, c2, kernel_size=1, stride=s)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.op = nn.Sequential(self.depth_conv, self.point_conv, self.bn, self.act)
    def forward(self, x):
        return self.op(x)
...
######################CA###########################
class DP_RepNBottleneck(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, kernels, groups, expand
        super(DP_RepNBottleneck, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = RepConvN(c1, c_, k[0], 1)
        self.cv2 = DP_Conv(c_, c2, k[1], 1, g=g)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        return x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))

class DP_RepNCSP(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(DP_RepNCSP, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(DP_RepNBottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

    def forward(self, x):
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1))

class DP_RepNCSPELAN4(nn.Module):
    # csp-elan
    def __init__(self, c1, c2, c3, c4, c5=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(DP_RepNCSPELAN4, self).__init__()
        self.c = c3//2
        self.cv1 = Conv(c1, c3, 1, 1)
        self.cv2 = nn.Sequential(DP_RepNCSP(c3//2, c4, c5), DP_Conv(c4, c4, 3, 1))
        self.cv3 = nn.Sequential(DP_RepNCSP(c4, c4, c5), DP_Conv(c4, c4, 3, 1))
...
class ADown(nn.Module):
    def __init__(self, c1, c2):  # ch_in, ch_out, shortcut, kernels, groups, expand
        super().__init__()
        self.c = c2 // 2
        self.cv1 = DP_Conv(c1 // 2, self.c, 3, 2, 1)
...
class Concat(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = [F.interpolate(tensor, size=(x[1].shape[2], x[1].shape[3]), mode='nearest') for tensor in x]
        # upsample = nn.Upsample(size=(x[1].shape[2], x[].shape[3]), mode='nearest')
        # t2 = upsample(t2)
...
class DRepConvN(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, p=1, g=1, d=1, act=True, deploy=False):
        super(DRepConvN, self).__init__()
        self.conv1 = depthConv(c1, c2, k, s, p)
        self.conv2 = depthConv(c1, c2, 1, s, 0)
        self.add = deploy and c1==c2
        self.act = nn.SiLU() if act else nn.Identity()

    def forward(self, x):
        return self.act(self.conv1(x) + self.conv2(x))

class DRepNBottleNeck(nn.Module):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3,3),e=0.5):
        super(DRepNBottleNeck, self).__init__()
        c_ = int(c2 * e)
        self.cv1 = DRepConvN(c1, c_, k[0], 1)
        self.cv2 = depthConv(c_, c2, k[1],1)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        return x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))

# class DRepNCSP(nn.Module):
#     def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
#         super(DRepNCSP, self).__init__()
#
#         c_ = int(c2 * e)
#         assert c_ > 0, f"Intermediate channel number must be positive, but got {c_}"
#         self.cv1 = depthConv(c1, c_, 1, 1)
#         self.cv2 = depthConv(c1, c_, 1, 1)
#         self.cv3 = depthConv(2*c_, c2, 1, 1)
#         self.m = nn.Sequential(*[DRepNBottleNeck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])
#
#     def forward(self, x):
#         return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)),1))
...
# class DRepNCSPELAN4(nn.Module):
#     def __init__(self, c1, c2, c3, c4, c5=1):
#         super(DRepNCSPELAN4, self).__init__()
#         assert c1 > 0 and c2 > 0 and c3 > 0 and c4 > 0, "All channel parameters must be positive integers"
#
#         self.c = c3 // 2
#         assert self.c > 0, f"c3 // 2 must be greater than zero, but got {self.c}"
#
#         self.cv1 = depthConv(c1, c3, 1, 1)
#         self.cv2 = nn.Sequential(DRepNCSP(c3 // 2, c4, c5), depthConv(c4, c4, 3, 1))
#         self.cv3 = nn.Sequential(DRepNCSP(c4, c4, c5), depthConv(c4, c4, 3, 1))
#         self.cv4 = depthConv(c3 + 2 * c4, c2, 1, 1)
#
#     def forward(self, x):
#         y = list(self.cv1(x).chunk(2,1))
#         y.extend((m(y[-1]) for m in [self.cv2, self.cv3]))
...
class DRepNCSPELAN4(nn.Module):
    def __init__(self, c1, c2, c3, c4, c5=1):
        super().__init__()
        self.c = c3 // 2
        print(f"RepNCSPELAN4: c1={c1}, c2={c2}, c3={c3}, c4={c4}, c5={c5}")
        print(f"Intermediate channel: self.c={self.c}")
    ...
    def forward(self, x):
        print(f"RepNCSPELAN4 forward: input shape={x.shape}")
        x = self.cv1(x)
        y = list(x.chunk(2, 1))
        print(f"chunk output shapes={[y_.shape for y_ in y]}")
        y.extend((m(y[-1]) for m in [self.cv2, self.cv3]))
        out = self.cv4(torch.cat(y, 1))
        print(f"RepNCSPELAN4 forward: output shape={out.shape}")
        return out
# class DRepNCSPELAN4(nn.Module):
#     def __init__(self, c1, c2, c3, c4, c5=1):
#         super().__init__()
...
class DepthRepNCSPELAN4(nn.Module):
    def __init__(self, c1, c2, blocks=4):
        super(DepthRepNCSPELAN4, self).__init__()
        self.c = c2//2

        self.conv1 = depthConv(c1, self.c)
        self.conv2 = depthConv(c1, self.c)
...
class RepNCSPELAN4(nn.Module):
    ...
    def forward(self, x):
        ...
        #     y = [F.interpolate(tensor, size=(y[0].shape[2], y[0].shape[3]), mode='nearest') for tensor in y]
        return self.cv4(torch.cat(y, 1))
