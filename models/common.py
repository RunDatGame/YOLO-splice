...
# GM_Conv模块
class GM_Conv(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, 3, 1, 1, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        # 防止 c2//2 为 0
        inner_channels = max(c2 // 2, 1)
        self.repncsp = RepNCSPELAN4(c2, c2, inner_channels, 1)
...
class RepConv(nn.Module):
    def __init__(self, c1, c2):
        ...
        self.conv2 = nn.Conv2d(c1, c2, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn2 = nn.BatchNorm2d(c2)

    def forward(self, x):
        return self.bn1(self.conv1(x)) + self.bn2(self.conv2(x))

# Conv-grad输出模块
class Conv_grad(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
...
#     def __init__(self, C_in, C_out, kernel_size=3, stride=1, padding=1, bias=True):
#         super(DepthConv, self).__init__()
#         self.depthwise = nn.Conv2d(C_in, C_in, kernel_size=kernel_size, stride=stride, padding=padding, groups=C_in, bias=bias)
#         # 点卷积
#         self.pointwise = nn.Conv2d(C_in, C_out, kernel_size=1, padding=0, bias=False),
#         # 归一化
#         self.BatchNorm = nn.BatchNorm2d(C_out, affine=True)
#         # 激活函数
#         self.re = nn.LeakyReLU()
#         self.op = nn.Sequential(self.depthwise, self.pointwise, self.BatchNorm, self.re)
#
#
#     def forward(self, x):
#         out = self.op(x)
...
class DP_RepNCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = DP_Conv(c1, c_, 1, 1)
        self.cv3 = DP_Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
...
class DP_RepNCSPELAN4(nn.Module):
    ...
    def __init__(self, c1, c2, c3, c4, c5=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv4 = DP_Conv(c3+(2*c4), c2, 1, 1)
