...
class DualDDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        self.cv5 = nn.ModuleList(
            nn.Sequential(Conv(x, c5, 3), Conv(c5, c5, 3), nn.Conv2d(c5, self.nc, 1)) for x in ch[self.nl:])
        # 分级分支
        self.cv_g2 = nn.ModuleList(
            nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch[:self.nl])

        # 将离散的分布回归转换为边界框坐标，提升回归精度。
        self.dfl = DFL(self.reg_max)
        self.dfl2 = DFL(self.reg_max)
...
class DualGDDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        c6 = max(ch[2 * self.nl], min(self.nc * 2, 128))
        ...
        # 分级分支
        self.cv_g1 = nn.ModuleList(
            nn.Sequential(Conv(x, c6, 3), Conv(c3, c6, 3), nn.Conv2d(c6, self.nc, 1)) for x in ch[2*self.nl: ])
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            d1.append(torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1))
            d2.append(torch.cat((self.cv4[i](x[self.nl+i]), self.cv5[i](x[self.nl+i])), 1))
            g1.append(self.cv_g3[i](x[2 * self.nl + i]))
        if self.training:
            return [d1, d2,  g3]
        elif self.dynamic or self.shape != shape:
            self.anchors, self.strides = (d1.transpose(0, 1) for d1 in make_anchors(d1, self.stride, 0.5))
        ...
        box2, cls2 = torch.cat([di.view(shape[0], self.no, -1) for di in d2], 2).split((self.reg_max * 4, self.nc), 1)
        dbox2 = dist2bbox(self.dfl2(box2), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
        grad3 = torch.cat([gi.view(shape[0], self.nc, -1) for gi in g1], 2).sigmoid()

        y1 = torch.cat((dbox, cls.sigmoid(), grad3), 1)
        y2 = torch.cat((dbox2, cls2.sigmoid(), grad3), 1)
        # y = [torch.cat((dbox, cls.sigmoid()), 1), torch.cat((dbox2, cls2.sigmoid()), 1)]
        return [y1, y2] if self.export else ([y1, y2], [d1, d2, g1])
        #y = torch.cat((dbox2, cls2.sigmoid()), 1)
        #return y if self.export else (y, d2)
        #y1 = torch.cat((dbox, cls.sigmoid()), 1)
        #y2 = torch.cat((dbox2, cls2.sigmoid()), 1)
...
class DP_DualDDetect(nn.Module):
    # YOLO Detect head for detection models
    dynamic = False  # force grid reconstruction
    export = False  # export mode
    shape = None
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        # 回归分支
        self.cv2 = nn.ModuleList(
            nn.Sequential(DP_Conv(x, c2, 3), DP_Conv(c2, c2, 3, g=4), nn.Conv2d(c2, 4 * self.reg_max, 1, groups=4)) for x in ch[:self.nl])
        # 分类分支
        self.cv3 = nn.ModuleList(
            nn.Sequential(DP_Conv(x, c3, 3), DP_Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch[:self.nl])

        ### nl: 第二组检测头
        # 回归分支
        self.cv4 = nn.ModuleList(
            nn.Sequential(DP_Conv(x, c4, 3), DP_Conv(c4, c4, 3, g=4), nn.Conv2d(c4, 4 * self.reg_max, 1, groups=4)) for x in ch[self.nl:])
        # 分类分支
        self.cv5 = nn.ModuleList(
            nn.Sequential(DP_Conv(x, c5, 3), DP_Conv(c5, c5, 3), nn.Conv2d(c5, self.nc, 1)) for x in ch[self.nl:])
        # 将离散的分布回归转换为边界框坐标，提升回归精度。
        self.dfl = DFL(self.reg_max)
        self.dfl2 = DFL(self.reg_max)
...
class BaseModel(nn.Module):
    ...
    def _profile_one_layer(self, m, x, dt):
        if isinstance(m, nn.Conv2d) and not hasattr(m, 'total_ops'):
            m.total_ops = 0
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, DualDetect, TripleDetect, DDetect, DualDDetect, DualGDDetect, DP_DualDDetect, TripleDDetect, Segment, DSegment, DualDSegment, Panoptic)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov9-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        super().__init__()
        ...
        if isinstance(m, (DualDetect, TripleDetect, DualDDetect, DP_DualDDetect, TripleDDetect, DualDSegment)):
            s = 256  # 2x min stride
            m.inplace = self.inplace
...
#             ELAN1, RepNCSPELAN4, SPPELAN, DP_RepNCSPELAN4, CoordAtt}:
#             c1, c2 = ch[f], args[0]
...
#             c2 = ch[f[-1]]
#         # TODO: channel, gw, gd
#         elif m in {Detect, DualDetect, TripleDetect, DDetect, DualDDetect, DP_DualDDetect, TripleDDetect, Segment, DSegment, DualDSegment, Panoptic}:
...
#     #         if c2 != no:  # if not output
#     #             c2 = make_divisible(c2 * gw, 8)
#     #             print(f'Layer {i}: module {m}, c1={c1}, c2={c2}')
#     #
#     #         args = [c1, c2, *args[1:]]
#     #
#     #         if m in {BottleneckCSP, SPPCSPC}:
...
#     #     # 新增对 Conv_grad 的支持：
#     #     elif m is Conv_grad:
#     #         # 这里直接从args取 c1, c2，不用ch[f]
#     #         c1, c2 = args[0], args[1]
#     #         args = [c1, c2, *args[2:]]
#     #
#     #     elif m in {Detect, DualDetect, TripleDetect, DDetect, DualDDetect, DP_DualDDetect, TripleDDetect, Segment,
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if isinstance(f, int):
            ...
            raise TypeError(f"Invalid 'from': {f}")

        # ========== 各模块通道处理 ==========
        if m in {
            Conv, AConv, ConvTranspose, Bottleneck, SPP, SPPF, DWConv, BottleneckCSP,
            nn.ConvTranspose2d, DWConvTranspose2d, SPPCSPC, ADown, ELAN1, RepNCSPELAN4,
            SPPELAN, DP_RepNCSPELAN4, CoordAtt
        }:
            c2 = args[0]
        ...
        elif m in {Detect, DualDetect, TripleDetect, DDetect, DualDDetect, DP_DualDDetect,
                   TripleDDetect, Segment, DSegment, DualDSegment, Panoptic}:
            args.append(c1 if isinstance(c1, list) else [c1])
            if m in {Segment, DSegment, DualDSegment, Panoptic}:
                args[2] = make_divisible(args[2] * gw, 8)
            c2 = None
        ...
        save.extend(x if isinstance(x, int) else x for x in ([f] if isinstance(f, int) else f) if x != -1)
        layers.append(m_)
        if c2 is not None:
            ch.append(c2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov9-c.yaml', help='model.yaml')
    ...
    # model = yolo
    # input = torch.randn(1, 3, 416, 416)  # Example input
